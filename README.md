...
| Git | Any | Cloning repos (optional) |

### GitHub Token (for real data)
...
```bash
# Required for GitHub data extraction
export GITHUB_TOKEN="ghp_your_token_here"

# Optional: Custom database
export DATABASE_URL="postgresql://user:pass@localhost/qc_model"
```
...
DATABASE_URL=sqlite:///data/qc_model.db
```
...
#### Option A: Demo Data (Quick Start)

Use synthetic data to test the pipeline without GitHub API access:

```bash
python scripts/generate_demo_data.py --n-prs 2000 --n-authors 50
```

This creates `data/filtered_prs.json` with realistic synthetic PR data.
...
# Force re-extraction (ignore cached data)
python scripts/run_pipeline.py extract --force
```

**Resume & Checkpoint Features:**
- **Automatic resume**: If extraction stops mid-way, just re-run the same command
- **Per-repo caching**: Completed repos are saved immediately and skipped on resume
- **In-progress checkpoints**: Every 100 PRs are checkpointed within a repo
- **Ctrl+C safe**: Pressing Ctrl+C saves a checkpoint before exiting
...
**What happens:**
- Completed repos are saved in `data/raw_{repo}.json`
- Current repo progress is checkpointed every 100 PRs in `data/checkpoint_{repo}.json`
...
**To start fresh:**
```bash
# Delete cached files and re-extract
rm data/raw_*.json data/checkpoint_*.json
python scripts/run_pipeline.py extract
